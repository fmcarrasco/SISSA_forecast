...
class gefs_class:
    def __init__(self, variable, fecha, nens):
        ...
        self.carpeta = '/Volumes/Almacenamiento/python_proyects/DATOS/GEFSv12/'
        ...
        if (f1 or f2):
            ...
            exit()
        self.variable_names = {'apcp_sfc':'precip', 'tmp_2m':'t2mean'}
        self.long_names = {'apcp_sfc':'daily precipitation 12UTC 12UTC', 'tmp_2m':'2m daily mean temperature'}
    ...
    def get_time(self):
        ...
        if self.variable == 'apcp_sfc':
            ...
            grupos = d1.groupby(pd.Grouper(freq='24H', origin=fechas[3], label='left')).ngroup().to_numpy()
            aux = pd.to_datetime(d1.groupby(pd.Grouper(freq='24H', origin=fechas[3], label='left')).nth(0)).to_list()
            fechas_d = [dt.datetime(a.year, a.month, a.day) for a in aux[1:-1]]
        self.fechas = fechas
        self.grupos = grupos
    ...
    def load_daylyfunc(self):
        ...
        self.longname_daily = {'tmp_2m':'mean temperature at 2m','apcp_sfc':'total precipitation'}
    ...
    def save_netcdf(self, ncfile):
        ...
        fvar.units = str(self.units)
        fvar.long_name = self.long_name
        fvar[:] = self.data
