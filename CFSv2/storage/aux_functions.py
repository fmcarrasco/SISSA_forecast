...
def open_archivo_viento(archivo):
...
def open_archivo_surface(archivo):
    ds = xr.open_dataset(archivo, engine='cfgrib',
...
def get_cut_grib(nfile):
    ...
    lon = (ds.coords['lon_0'].to_numpy() +180) % 360 - 180
    lat = ds.coords['lat_0'].to_numpy()
    # Lon/Lat BOUNDS
    ...
    for index, row in variables.iterrows():
        datos[row['var']] = ds[row['vargrb2']].to_numpy()
        unidades[row['var']] = ds[row['vargrb2']].units 
